...
# --- TFLite Micro sources (C++ only for now) ---
TFLM_CC_SRCS := \
  $(abspath tflite_micro/tensorflow/lite/micro/all_ops_resolver.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/micro_allocator.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/micro_error_reporter.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/micro_interpreter.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/micro_utils.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/kernels/cmsis-nn/fully_connected.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/kernels/reshape.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/kernels/logistic.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/memory_helpers.cc) \
  $(abspath tflite_micro/tensorflow/lite/core/api/tensor_utils.cc)
  
  # TFLM C++ sources required by missing symbols
TFLM_CC_SRCS += \
  $(abspath tflite_micro/tensorflow/lite/core/api/error_reporter.cc) \
  $(abspath tflite_micro/tensorflow/lite/core/api/flatbuffer_conversions.cc) \
  $(abspath tflite_micro/tensorflow/lite/core/api/op_resolver.cc) \
  $(abspath tflite_micro/tensorflow/lite/kernels/internal/quantization_util.cc) \
  $(abspath tflite_micro/tensorflow/lite/kernels/kernel_util.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/memory_planner/greedy_memory_planner.cc) \
  $(abspath tflite_micro/tensorflow/lite/micro/simple_memory_allocator.cc)
  
# TFLM C sources required by missing symbols
TFLM_C_SRCS += \
  $(abspath tflite_micro/tensorflow/lite/c/common.c) \
  $(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/NN/Source/FullyConnectedFunctions/arm_fully_connected_s8.c) \
#  $(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/NN/Source/NNSupportFunctions/arm_nn_vec_mat_mult_t_s8.c) \
  $(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/NN/Source/NNSupportFunctions/arm_nn_vec_mat_mult_t_s8.c)

TFLM_BUILD := $(abspath build/tflm)
TFLM_LIB   := $(TFLM_BUILD)/libtflm.a

# create object filenames from source filenames reliably
TFLM_CC_OBJS := $(patsubst $(abspath tflite_micro)/%.cc,$(TFLM_BUILD)/%.o,$(TFLM_CC_SRCS))
TFLM_C_OBJS  := $(patsubst $(abspath tflite_micro)/%.c,$(TFLM_BUILD)/%.o,$(TFLM_C_SRCS))
TFLM_OBJS    := $(TFLM_CC_OBJS) $(TFLM_C_OBJS)

# Debug print to confirm object list while running make
$(info TFLM_CC_SRCS = $(TFLM_CC_SRCS))
$(info TFLM_CC_OBJS = $(TFLM_CC_OBJS))
$(info TFLM_C_SRCS  = $(TFLM_C_SRCS))
$(info TFLM_C_OBJS  = $(TFLM_C_OBJS))

# Compile rules
$(TFLM_BUILD)/%.o: $(abspath tflite_micro)/%.cc
	@mkdir -p $(dir $@)
	$(RISCV_GXX) $(RISCV_CXXFLAGS) -c $< -o $@

$(TFLM_BUILD)/%.o: $(abspath tflite_micro)/%.c
	@mkdir -p $(dir $@)
	$(RISCV_GCC) $(RISCV_CFLAGS) -c $< -o $@

# Archive into a static library (fail early if no objects)
$(TFLM_LIB): $(TFLM_OBJS)
	@mkdir -p $(dir $@)
	@if [ -z "$(TFLM_OBJS)" ]; then echo "Error: TFLM_OBJS is empty. Check TFLM_CC_SRCS paths."; false; fi
	$(RISCV_AR) rcs $@ $^
...
# Include the TFLM headers
RISCV_CFLAGS    += -I$(abspath tflite_micro/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/)
RISCV_CFLAGS    += -I$(abspath tflite_micro/third_party/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/third_party/)
RISCV_CFLAGS    += -I$(abspath tflite_micro/third_party/gemmlowp/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/third_party/gemmlowp/)
RISCV_CFLAGS    += -I$(abspath tflite_micro/third_party/flatbuffers/include/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/third_party/flatbuffers/include/)
RISCV_CFLAGS    += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/)
RISCV_CFLAGS    += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/NN/Include/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/NN/Include/)
RISCV_CFLAGS    += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/NN/Source/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/NN/Source/)
RISCV_CFLAGS    += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/Core/Include/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/Core/Include/)
RISCV_CFLAGS    += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/DSP/Include/)
RISCV_CXXFLAGS  += -I$(abspath tflite_micro/tensorflow/lite/micro/tools/make/downloads/cmsis/CMSIS/DSP/Include/)


# Bare-metal C++: no exceptions, no RTTI, no unwind tables
RISCV_CXXFLAGS += -fno-exceptions -fno-rtti -fno-unwind-tables -fno-asynchronous-unwind-tables -fno-threadsafe-statics
RISCV_CFLAGS   += -fno-unwind-tables -fno-asynchronous-unwind-tables
...
# Explicitly add the BSP (metal) install lib directory so -lmetal and -lmetal-gloss are found
RISCV_LDFLAGS += -L$(abspath $(BSP_DIR)/install/lib/$(CONFIGURATION))
# Ensure the linker uses the correct metal LDS (use the one from the BSP)
# (this references the metal script that your PROGRAM_ELF rule already lists as a prerequisite)
RISCV_LDFLAGS += -T$(abspath $(BSP_DIR)/metal.$(LINK_TARGET).lds)
# Add the TFLite Micro build directory to the linker search path (only once)
RISCV_LDFLAGS += -L$(abspath $(TFLM_BUILD))

# Link to the relevant libraries
RISCV_LDLIBS += -Wl,--start-group -lc -lgcc -lm -lmetal $(LIBMETAL_EXTRA) -lstdc++ -Wl,--end-group

# Link TFLM by name (ensure libtflm.a exists in $(TFLM_BUILD))
RISCV_LDLIBS  += -ltflm

# The C++ standard library must be explicitly linked for TFLite Micro C++ features.
# This must be added to the linker libraries.
RISCV_LDLIBS += -lstdc++
